...
#include <float.h>  // Diperlukan untuk FLT_MAX atau FLT_MIN jika inisialisasi ekstrem digunakan (alternatif: inisialisasi dengan nilai pertama)
...
int main() {
    ...
    int prev_num;   // Variabel baru untuk menyimpan angka sebelumnya, diperlukan untuk rata-rata pasangan
    int min_val;    // Variabel untuk menyimpan nilai terkecil
    int max_val;    // Variabel untuk menyimpan nilai terbesar
    float min_avg;  // Variabel baru untuk menyimpan nilai rata-rata terendah dari pasangan
    int i;          // Variabel counter untuk loop

    int first_num_read = 0;   // Flag untuk inisialisasi min_val dan max_val dengan angka pertama
    int first_pair_read = 0;  // Flag baru untuk inisialisasi min_avg dengan rata-rata pasangan pertama
    ...
    // --- Bagian 2: Membaca 'n' baris masukan dan mencari min/max serta rata-rata terendah ---
    ...
    for (i = 0; i < n; i++) {
        ...
        if (i >= 1) { 
            ...
            // Inisialisasi min_avg dengan rata-rata pasangan pertama yang ditemukan
            if (!first_pair_read) {
                min_avg = current_avg;
                first_pair_read = 1;
            } else {
